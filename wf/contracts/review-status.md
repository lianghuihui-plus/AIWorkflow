...
- 产物进入 `待审核` 后，`CONTEXT.md` 的下一步必须写为 `review-artifact`，且该产物必须出现在 `CONTEXT.md` 的 `待处理产物` 列表中。
- 用户通过对话提出修改意见时，Agent 必须先写入 `REVISIONS.md`，并将目标产物审核状态改为 `需修改`。
- 产物进入 `需修改` 后，`CONTEXT.md` 的下一步必须保持 `review-artifact`，且该产物必须出现在 `CONTEXT.md` 的 `待处理产物` 列表中。
- `wf` 处理完相关修订后，目标产物审核状态仍为 `待审核`，等待用户重新确认。
- 只有用户明确确认通过，且目标产物通过对应产物契约校验后，才能将状态写为 `已确认`，并填写审核人和审核时间。
- 未达到 `已确认` 的产物不得作为进入下一阶段的门禁通过依据。
- 产物审核状态改为 `已确认` 后，运行时必须根据所有阶段产物审核状态重建 `CONTEXT.md` 的 `待处理产物` 列表。

...
- 不得只根据任务标题或需求摘要生成规格。
- 规格保留正常路径、边界条件、异常处理和状态变化等可验证行为，但修改点只能描述业务源码、配置、资源、接口和数据结构变更。
- 测试或回归影响不得转换为测试文件、测试类、Mock、断言或测试用例修改点。
...
- 基于规格的关键行为和约束、实现报告以及真实代码变更生成或更新单元测试。
- 默认只修改测试代码和测试报告。
- 暂未生成的测试必须记录原因和后续条件。
- 测试报告的测试项、未覆盖原因和辅助验证记录使用列表块记录，避免把长行为、命令、路径或说明塞进横向表格。

...
write_base_workspace() {
  local ws="$1"
  mkdir -p "$ws/prd" "$ws/output/specs"
  printf '需求\n' > "$ws/prd/req.md"
  cat > "$ws/AGENT.md" <<'EOF'
# Agent
EOF
...
EOF
  write_review_status "$ws/output/specs/T-001.md" "已确认"
  write_review_status "$ws/output/report-T-001.md" "已确认"
  write_review_status "$ws/output/test-report-T-001.md" "已确认"

  python3 "$ROOT_DIR/wf/tools/rebuild_context.py" "$ws"
  if grep -q "| T-001 | output/test-report-T-001.md | ✅ 已完成 |" "$ws/CONTEXT.md"; then

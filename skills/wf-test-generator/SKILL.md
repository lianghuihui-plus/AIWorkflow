...
description: 测试生成。读取 spec 规格文件和实际代码，对照关键行为生成单元测试。过程记录写入 CONTEXT/ISSUES/JOURNAL。
---

# 测试生成

对照 spec 中的关键行为，为已生成的代码编写单元测试。
...
- 在 `规格` 索引中标注测试完成（`T-001 — ✅ 已测试`）
- 在 `## 测试记录` 表格中追加（`T-001 | tests/AuthService.test.ets | ✅ 已完成`）

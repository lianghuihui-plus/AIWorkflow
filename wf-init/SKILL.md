...
如果当前 AIWorkFlow 安装中存在 `wf/tools/render_review_dashboard.py`，初始化完成后应使用它生成根目录 `dashboard.html`。该文件是脚本生成的人工检视快照，可纳入版本管理，但不作为流程事实源。
...
- **是文件**：记录该文件绝对路径。
- **是目录**：扫描目录下所有普通文件，不限制扩展名，逐条记录绝对路径；子目录不作为 PRD 文件。
- 不向用户确认，直接进入下一问。
...
工作空间文件生成完成后，如果可定位当前 `wf` skill 目录下的 `tools/render_review_dashboard.py`，执行：

```text
python <wf-skill-dir>/tools/render_review_dashboard.py <workspace>
```

生成工作空间根目录 `dashboard.html`。该步骤为 best-effort：失败时不回滚初始化，只在完成提示中说明检视页生成失败和错误摘要。
...
📄 已生成 AGENT.md
📄 已初始化 CHANGELOG.md
📄 已生成 dashboard.html（人工检视页，如生成失败则提示失败原因）

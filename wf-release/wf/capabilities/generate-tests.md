...
- 从规格的 `关键行为` 和约束、实现报告以及真实代码变更中提取待覆盖行为；规格中的业务代码修改点只作为定位上下文，不作为测试实现指令。
- 如果历史规格包含测试代码修改点，不继承其中的测试文件、Mock 或断言设计，应按项目现有测试模式重新生成，并提示规格存在阶段越界内容。
- 遵循代码仓库已有单元测试框架、目录、命名风格和 mock 方式。
- 为可测试行为生成或更新单元测试，覆盖正常路径、边界条件、异常处理和状态变化。
- 对暂未生成单元测试的行为，在测试报告中记录测试点、状态、原因和后续条件。
- 用列表块记录每个测试项和辅助验证项；不要把行为、测试点、说明、命令或长路径塞进横向 Markdown 表格。
- 测试报告中只允许短字段汇总使用表格；测试项明细、未覆盖原因和验证记录默认按 `### 序号. 标题` + 字段列表书写。
